...
class QnnStableDiffusionApp:
    ...
    def __init__(
        self,
        num_steps: int = 50,
        seed: int = np.int64(1.36477711e+14),
        ...
        self.time_embeddings = UNet2DConditionModel.from_pretrained('runwayml/stable-diffusion-v1-5',
                                                       subfolder='unet').time_embedding
        ...
        self.scheduler = DPMSolverMultistepScheduler(num_train_timesteps=1000, beta_start=0.00085,
    ...
    def generate_image(
        ...
        random_init_latent = torch.randn((1, 4, 64, 64), generator=torch.manual_seed(self.seed)).numpy()
        ...
        for step in range(self.num_steps):
            ...
            unconditional_noise_pred = self.run_unet(latent_in, self.get_time_embedding(timestep), uncond_text_embedding)

            # Run U-net for user text embeddings
            conditional_noise_pred = self.run_unet(latent_in, self.get_time_embedding(timestep), user_text_embedding)
            
            # Run Scheduler
            latent_in = self.run_scheduler(unconditional_noise_pred, conditional_noise_pred, latent_in, timestep)
    ...
    def run_scheduler(self, noise_pred_uncond, noise_pred_text, latent_in, timestep):
        ...
        # Run Scheduler step
        latent_out = self.scheduler.step(noise_pred, timestep, latent_in).prev_sample.numpy()
    ...
    def run_text_encoder(self, input_data):
        output_data = self.run_qnn_net_run(f'{self.models_context_path}\\stable_diffusion_v1_5_quantized-textencoder_quantized-snapdragon_x_elite.bin', [input_data])
    ...
    def run_unet(self, input_data_1, input_data_2, input_data_3):
        output_data = self.run_qnn_net_run(f'{self.models_context_path}\\stable_diffusion_v1_5_quantized-unet_quantized-snapdragon_x_elite.bin', [input_data_1, input_data_2, input_data_3])
    ...
    def run_vae(self, input_data):
        output_data = self.run_qnn_net_run(f'{self.models_context_path}\\stable_diffusion_v1_5_quantized-vaedecoder_quantized-snapdragon_x_elite.bin', [input_data])
...
class QnnStableDiffusionExecutor(LLMExecutor):
    ...
    def extend_arguments(self, parser):
        ...
        model_group.add_argument('--qnn_binaries_path', type=str, default=self.qnn_binary_path, help='Path to QNN binaries.')
        model_group.add_argument('--model_path', type=str, default=self.model_path, help='Path to model binaries.')
    ...
    async def llm_compute(self, history: list[dict], modelfile:Modelfile):
        seed = modelfile.parameters["imgen_"].get("seed", np.int64(1.36477711e+14))
        num_inference_steps = modelfile.parameters["imgen_"].get("num_inference_steps", 20)
