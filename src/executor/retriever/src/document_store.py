...
class DocumentStore:
    ...
    def __init__(
        self, vector_store: FAISS, embedding_model: HuggingFaceEmbeddings = None
    ):
    ...
    def load(
        cls,
        path: str,
        embedding_model_store: EmbeddingModelStore = EmbeddingModelStore(),
    ):
        """
        ...
        store = cls(
            embedding_model=embedding_model_store.load_model(embedding_model_name)
        )
...
class DocumentStoreFactory:
    ...
    async def from_documents(self, docs: [Document]):
        ...
        vector_store = await loop.run_in_executor(
            None, FAISS.from_documents, chunks, self.embedding_model
        )
