...
class Diary:
    ...
    segment_template = {"start_time": 0, "end_time": 0, "text": "", "speaker": []}
    ...
    def insert(self, start: float, end: float, speaker=None):
        if (end - start) <= self.duration_thld_sec:
            return
    ...
    def annotate_transcript(self, transcript: [dict]):
        ...
        if len(transcript) == 0:
            return []
        if len(self.diary) == 0:
            self.insert(start=0, end=float("inf"), speaker="SPEAKER_OO")
        last_segment = self.segment_template.copy()
        last_segment["speaker"] = [
            sorted(self.diary, key=lambda x: x["start"])[0]["speaker"]
        ]
        result = []
        for segment in transcript:
            ...
            if (
                ...
                or len(speaker) == 0
                or segment["text"] in self.punctuation
            ):
...
class SpeakerDiarizer:
    ...
    def diarization(self, src_audio_file: str, num_speakers: int, **kwargs) -> [dict]:
        ...
        raise NotImplementedError(
            "You should implement this method for different speaker diarization."
        )
...
class PyannoteSpeakerDiarizer(SpeakerDiarizer):
    ...
    def _load_pipeline(self, device: str = None):
        self.pipeline = pyannote.audio.Pipeline.from_pretrained(self.pipeline_name)
        if device is None:
    ...
    def diarization(
        self, src_audio_file: str, num_speakers: int, device: str, **kwargs
    ) -> [dict]:
        ...
        for turn, _, speaker in diarization.itertracks(yield_label=True):
            result.insert(start=turn.start, end=turn.end, speaker=speaker)
