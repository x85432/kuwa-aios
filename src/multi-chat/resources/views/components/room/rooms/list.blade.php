...
@elseif (session('llms'))
    ...
    <div class="flex flex-1 flex-col border border-black dark:border-white border-1 rounded-lg overflow-hidden">
        <div
            ...
            @foreach ($llms as $llm)
                <div
                    class="mx-1 overflow-x-auto flex-shrink-0 flex items-center justify-center">
                    <div data-tooltip-target="{{ $extra }}llm_{{ $llm->id }}_list"
                        data-tooltip-placement="top" class="relative h-5 w-5">
                        <img class="h-full h-full overflow-hidden rounded-full bg-black"
                        ...
                        <div class="absolute bottom-0 right-0 z-2 opacity-90">
                            @if (!$llm->healthy || time() - strtotime($llm->updated_at) > 300)
                                <div class="bg-red-500 rounded-full w-2 h-2" data-updated-at="{{$llm->updated_at}}"></div>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    @if ($llms && $DC)
        ...
        <div class="flex flex-1 flex-col border border-black dark:border-white border-1 rounded-lg overflow-hidden">
            ...
            $join->on('llms.id', '=', 'bots.model_id');
                    ...
                    <div
                        class="mx-1 overflow-x-auto flex-shrink-0 flex items-center justify-center">
                        <div data-tooltip-target="{{ $extra }}llm_{{ $chat->bot_id }}_list"
                            data-tooltip-placement="top" class="relative h-5 w-5">
                            <img class="h-full h-full overflow-hidden rounded-full bg-black"
                                src="{{ $chat->image ?? $chat->base_image ? asset(Storage::url($chat->image ?? $chat->base_image)) : '/' . config('app.LLM_DEFAULT_IMG') }}">

                            <div class="absolute bottom-0 right-0 z-2 opacity-90">
                                @if (!$chat->healthy || time() - strtotime($chat->updated_at) > 300)
                                    <div class="bg-red-500 rounded-full w-2 h-2" data-updated-at="{{$chat->updated_at}}"></div>
                                @endif
                            </div>
                        </div>
                    </div>
